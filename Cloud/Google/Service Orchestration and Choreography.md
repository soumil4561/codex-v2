...
A fully managed service that allows to manage the execution, dispatch and delivery of a large number of distributed tasks. We can **asynchronously** perform work outside of a user request. The tasks can be executed on App Engine or any arbitrary HTTP endpoint.

Cloud task is explicit invocation, meaning task creator (or event emitter) has full control over the task. On the contrary pub sub is implicit invocation, so any subscriber will be invoked when an event is generated and no specific control is by the pub sub event bus
_Pub/Sub should be used for event-based architectures that rely on receiving services reacting to events generated by other services._

![[Service Orchestration and Choreography-1764244340987.webp]]

### Cloud Scheduler
A fully managed, enterprise-grade cron job scheduler which can be managed from a single dashboard. Cloud Scheduler is used to schedule jobs that should be executed on a defined schedule or at regular intervals.
...
| hour         | 0-23                                                                                                                                                                                                                                        |
| day of month | 1-31                                                                                                                                                                                                                                        |
| month        | - 1-12, where 1 is January, 2 is February, and so on.<br>- Uppercase, lowercase and mixed-case three character strings, based on the English name of the month. For example: jan, feb, mar, apr, may, jun, jul, aug, sep, oct, nov, or dec. |
| day of week  | - 0-7, where 0 or 7 is Sunday, 1 is Monday, and so on.<br>- Uppercase, and lowercase or mixed-case three character strings, based on the English name of the day: mon, tue, wed, thu, fri, sat, or sun.                                     |
...
**A field can contain a single number, a range of numbers, or a list of numbers and ranges.**
...
![[Service Orchestration and Choreography-1764245258370.webp]]


## Cheat Sheet
![[Service Orchestration and Choreography-1764245356624.webp]]